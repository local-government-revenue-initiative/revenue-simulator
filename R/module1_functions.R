...
suggest_column_mapping <- function(df, expected_columns) {
  ...
  column_variations <- list(
    ...
    domestic_use_groundfloor = c(
      "domestic_use_groundfloor",
      "domestic_use_of_groundfloor"
    ),
    made_payment = c(
      "made_payment",
      "payment",
...
validate_column_mapping <- function(df, mapping, required_columns) {
  ...
  for (col in required_columns) {
    if (
      is.null(mapping[[col]]) ||
        mapping[[col]] == "" ||
        !mapping[[col]] %in% names(df)
    ) {
      errors <- c(
        errors,
        paste("Required column", col, "not mapped or invalid")
      )
    }
  }
  ...
  if (length(mapped_columns) != length(unique(mapped_columns))) {
    ...
    errors <- c(
      errors,
      paste(
        "Duplicate column mappings found:",
...
process_property_data <- function(df, column_mapping) {
  ...
  if (any(duplicated(clean_mapping))) {
    stop(
      "Duplicate column mappings detected. Each CSV column can only be mapped once."
    )
  }
  ...
  for (standard_name in names(clean_mapping)) {
    ...
    if (
      !is.null(old_name) && old_name != "" && old_name %in% names(df_renamed)
    ) {
...
merge_datasets <- function(
  ...
  property_data[[property_id_col]] <- as.character(property_data[[
    property_id_col
  ]])
  payment_data[[payment_id_col]] <- as.character(payment_data[[payment_id_col]])
  business_data[[business_id_col]] <- as.character(business_data[[
    business_id_col
  ]])
  ...
  if (nrow(business_data) > 0 && "property_type" %in% names(merged_data)) {
    ...
    if (nrow(props_with_businesses) > 0) {
      ...
      merged_data <- bind_rows(
        props_without_businesses,
        props_with_businesses_final
      )
    } else {
    ...
    merged_data <- merged_data %>%
      left_join(
        business_data, # Keep all columns including id_business!
