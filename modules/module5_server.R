  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$run_analysis, {
      ...
      withProgress(message = 'Running tax burden analysis...', value = 0, {
        ...
        values$progressivity_results <- calculate_progressivity(
        ...
        values$concentration_results <- calculate_tax_concentration(
          values$aggregated_data
        )
    ...
    output$effective_rate_box <- renderValueBox({
      if (!values$analysis_complete) {
        valueBox(
          ...
          subtitle = "Avg Effective Rate",
          icon = icon("percentage"),
          color = "black"
        )
      } else {
        rate <- (sum(values$aggregated_data$total_tax) /
          sum(values$aggregated_data$total_property_value)) *
          100
        valueBox(
    ...
    output$quantile_burden_table <- DT::renderDataTable({
      ...
      values$progressivity_results %>%
        ...
        DT::formatCurrency(
          columns = c('Total Value', 'Total Tax', 'Avg Value', 'Avg Tax'),
          currency = "",
          interval = 3,
          mark = ","
        ) %>%
    ...
    output$progressivity_index_plot <- renderPlot({
      ...
      ggplot(
        ...
        aes(
          x = value_quantile,
          y = progressivity_index,
          fill = progressivity_index
        )
      ) +
        ...
        labs(
          title = "Progressivity Index by Quantile",
    ...
    output$tax_value_share_plot <- renderPlot({
      ...
      ggplot(
        ...
        scale_fill_manual(
          values = c("value_share" = "#3498db", "tax_share" = "#e74c3c"),
          labels = c("Value Share", "Tax Share")
        ) +
        scale_y_continuous(labels = function(x) paste0(x, "%")) +
        labs(
          title = "Distribution of Property Value vs Tax Burden",
          x = "Property Value Quantile",
          y = "Share (%)",
          fill = ""
        ) +
    ...
    output$progressivity_metrics <- renderPrint({
      ...
      cat(
        "Total Properties:",
        format(stats$total_properties, big.mark = ","),
        "\n"
      )
      cat(
        "Total Property Value:",
        format(round(stats$total_value), big.mark = ","),
        "\n"
      )
      cat(
        "Total Tax Revenue:",
        format(round(stats$total_tax_revenue), big.mark = ","),
        "\n"
      )
      cat(
        "Overall Effective Rate:",
        round(stats$overall_effective_rate, 2),
        "%\n"
      )
      cat(
        "Gini Coefficient:",
      ...
      prog_index_q5 <- values$progressivity_results$progressivity_index[
        input$n_quantiles
      ]

      cat("Lowest Quantile Progressivity Index:", round(prog_index_q1, 2), "\n")
      cat(
        "Highest Quantile Progressivity Index:",
        round(prog_index_q5, 2),
        "\n\n"
      )
      ...
      if (prog_index_q1 < 0.8 && prog_index_q5 > 1.2) {
        ...
        cat(
          "Assessment: Tax system appears REGRESSIVE (low-value properties overtaxed)\n"
        )
      } else {
    ...
    output$effective_rate_scatter <- renderPlot({
      ...
      ggplot(
        ...
        geom_hline(
          yintercept = input$overtax_threshold,
          linetype = "dashed",
          color = "red",
          linewidth = 1
        ) +
        ...
        labs(
          title = "Effective Tax Rate vs Property Value",
          subtitle = paste(
            "Red line indicates overtaxation threshold of",
            input$overtax_threshold,
            "%"
          ),
    ...
    output$effective_rate_boxplot <- renderPlot({
      ...
      ggplot(
        ...
        labs(
          title = "Distribution of Effective Tax Rates by Quantile",
          x = "Property Value Quantile",
          y = "Effective Tax Rate (%)",
          fill = "Quantile"
        ) +
    ...
    output$effective_rate_by_type <- renderPlot({
      ...
      ggplot(
        type_data,
        aes(x = reorder(main_type, avg_rate), y = avg_rate, fill = main_type)
      ) +
        geom_bar(stat = "identity", alpha = 0.7) +
        geom_text(
          aes(label = paste0(round(avg_rate, 2), "%\n(n=", n, ")")),
          vjust = -0.5,
          size = 3.5
        ) +
        scale_fill_manual(
        ...
        labs(
          title = "Average Effective Tax Rate by Property Type",
          x = "Property Type",
      ...
      values$overtaxed_properties %>%
        ...
        DT::formatStyle(
          ...
          backgroundColor = DT::styleEqual(
            c(
              "High Risk - Low Value with Business",
              "High Risk - Low Value",
              "Medium Risk"
            ),
    ...
    output$overtax_by_quantile <- renderPlot({
      ...
      ggplot(
        overtax_summary,
        aes(x = value_quantile, y = pct_overtaxed, fill = pct_overtaxed)
      ) +
        geom_bar(stat = "identity", alpha = 0.8) +
        geom_text(
          aes(
            label = paste0(
              round(pct_overtaxed, 1),
              "%\n(",
              n_overtaxed,
              "/",
              n_total,
        ...
        labs(
          title = paste(
            "Properties with Effective Tax Rate >",
            input$overtax_threshold,
            "%"
          ),
          x = "Property Value Quantile",
    ...
    output$combined_burden_plot <- renderPlot({
      ...
      burden_data <- values$data_with_quantiles %>%
        ...
        tidyr::pivot_longer(
          cols = c(avg_property_tax, avg_business_license),
          names_to = "tax_type",
          values_to = "amount"
        )
      ...
      ggplot(
        burden_data,
        aes(x = value_quantile, y = amount, fill = tax_type)
      ) +
        ...
        labs(
          title = "Average Combined Tax Burden by Quantile and Business Status",
          x = "Property Value Quantile",
    ...
    output$business_burden_table <- DT::renderDataTable({
      ...
      DT::datatable(
        ...
        DT::formatRound(
          columns = c('avg_effective_rate', 'median_effective_rate'),
          digits = 2
        ) %>%
        DT::formatCurrency(
          columns = c('total_tax', 'avg_property_tax', 'avg_business_license'),
          currency = "",
          interval = 3,
          mark = ","
        )
    ...
    output$winners_losers_table <- DT::renderDataTable({
      ...
      DT::datatable(
        ...
        DT::formatCurrency(
          columns = names(winners_losers)[grepl(
            "total_tax",
            names(winners_losers)
          )],
          currency = "",
          interval = 3,
          mark = ","
        ...
        DT::formatRound(
          columns = names(winners_losers)[grepl("avg_", names(winners_losers))],
          digits = 2
        )
    })
    ...
    output$property_value_comparison <- renderPlot({
      ...
      value_data <- values$property_comparison %>%
        ...
        dplyr::mutate(
          ...
          Scenario = factor(
            Scenario,
            levels = c("Existing", "Scenario_A", "Scenario_B")
          )
        )

      ggplot(value_data, aes(x = Scenario, y = Value, fill = Scenario)) +
        geom_bar(stat = "identity", alpha = 0.8) +
        geom_text(aes(label = scales::comma(Value)), vjust = -0.5, size = 4) +
        scale_fill_manual(
          values = c(
            "Existing" = "#95a5a6",
            "Scenario_A" = "#3498db",
        ...
        scale_y_continuous(labels = scales::comma) +
        labs(
          title = "Total Property Value Comparison",
          x = NULL,
          y = "Property Value"
        ) +
        theme_minimal() +
    ...
    output$property_tax_comparison <- renderPlot({
      ...
      tax_data <- values$property_comparison %>%
        dplyr::filter(
          Metric %in% c("Total Property Tax", "Total Business License")
        ) %>%
        tidyr::pivot_longer(
          cols = c(Existing, Scenario_A, Scenario_B),
          names_to = "Scenario",
      ...
      ggplot(tax_data, aes(x = Scenario, y = Value, fill = Metric)) +
        ...
        scale_y_continuous(labels = scales::comma) +
        labs(
          title = "Total Tax Comparison (Property + Business)",
          x = NULL,
          y = "Tax Amount",
          fill = "Tax Type"
        ) +
    ...
    # ============================================
    # Quantile Impact Examples Tab
    # ============================================

    # Reactive value to store quantile analysis results
    values$quantile_representatives <- NULL
    values$quantile_analysis_complete <- FALSE

    # Update filter choices when revenue data is available
    observe({
      req(revenue_data())
    ...
    observeEvent(input$reset_quantile_filters, {
      updateSelectInput(
        session,
        "quantile_filter_structure_types",
        selected = "All"
      ...
      updateNumericInput(session, "quantile_n_quantiles", value = 5)
    })

    # Run quantile analysis
    observeEvent(input$run_quantile_analysis, {
      req(revenue_data())

      withProgress(
        message = 'Analyzing representative properties...',
        value = 0,
        {
          incProgress(
            0.3,
            detail = "Finding median properties in each quantile"
          )

          # Get filter values
          structure_filter <- if (
            is.null(input$quantile_filter_structure_types) ||
              "All" %in% input$quantile_filter_structure_types
          ) {
            "All"
          } else {
            input$quantile_filter_structure_types
          }

          property_filter <- if (
            is.null(input$quantile_filter_property_types) ||
              "All" %in% input$quantile_filter_property_types
          ) {
            "All"
          } else {
            input$quantile_filter_property_types
          }

          category_filter <- if (
            is.null(input$quantile_filter_license_categories) ||
              "All" %in% input$quantile_filter_license_categories
          ) {
            "All"
          } else {
            input$quantile_filter_license_categories
          }

          subcategory_filter <- if (
            is.null(input$quantile_filter_license_subcategories) ||
              "All" %in% input$quantile_filter_license_subcategories
          ) {
            "All"
          } else {
            input$quantile_filter_license_subcategories
          }

          incProgress(0.5, detail = "Calculating tax impacts")

          # Find representative properties
          values$quantile_representatives <- find_quantile_representatives(
            revenue_data = revenue_data(),
            n_quantiles = input$quantile_n_quantiles,
            filter_structure_types = structure_filter,
            filter_property_types = property_filter,
            filter_license_categories = category_filter,
            filter_license_subcategories = subcategory_filter
          )

          incProgress(0.9, detail = "Preparing visualizations")

          # Mark analysis as complete
          values$quantile_analysis_complete <- TRUE
        }
      )

      showNotification(
        "Quantile analysis complete!",
        type = "message",
        duration = 3
      )
    })
    ...
    output$quantile_analysis_complete <- reactive({
      values$quantile_analysis_complete
    })
    outputOptions(
      output,
      "quantile_analysis_complete",
      suspendWhenHidden = FALSE
    )

    # Value boxes
    output$quantile_properties_analyzed <- renderValueBox({
      req(values$quantile_representatives)

      if ("message" %in% names(values$quantile_representatives)) {
        valueBox(
          value = "0",
          subtitle = "Representative Properties",
          icon = icon("home"),
          color = "red"
        )
      } else {
        valueBox(
          value = nrow(values$quantile_representatives),
          subtitle = "Representative Properties Analyzed",
          icon = icon("home"),
          color = "blue"
        )
      }
    ...
    output$quantile_avg_change_a <- renderValueBox({
      req(values$quantile_representatives)

      if ("message" %in% names(values$quantile_representatives)) {
        valueBox(
          value = "N/A",
          subtitle = "Avg Change - Scenario A",
          icon = icon("arrow-up"),
          color = "black"
        )
      } else {
        avg_change <- mean(
          values$quantile_representatives$scenario_a_tax_change,
          na.rm = TRUE
        )
        avg_change_pct <- mean(
          values$quantile_representatives$scenario_a_tax_change_pct,
          na.rm = TRUE
        )

        valueBox(
          value = paste0(scales::comma(round(avg_change, 0))),
          subtitle = paste0(
            "Avg Tax Change - Scenario A (",
            round(avg_change_pct, 1),
            "%)"
          ),
          icon = icon(if (avg_change >= 0) "arrow-up" else "arrow-down"),
          color = if (abs(avg_change_pct) < 5) {
            "green"
          } else if (abs(avg_change_pct) < 15) {
            "yellow"
          } else {
            "red"
          }
    ...
    output$quantile_avg_change_b <- renderValueBox({
      req(values$quantile_representatives)

      if ("message" %in% names(values$quantile_representatives)) {
        valueBox(
          value = "N/A",
          subtitle = "Avg Change - Scenario B",
          icon = icon("arrow-up"),
          color = "black"
        )
      } else {
        avg_change <- mean(
          values$quantile_representatives$scenario_b_tax_change,
          na.rm = TRUE
        )
        avg_change_pct <- mean(
          values$quantile_representatives$scenario_b_tax_change_pct,
          na.rm = TRUE
        )
    ...
    # Visualizations
    output$quantile_tax_plot <- renderPlot({
      req(values$quantile_representatives)

      if ("message" %in% names(values$quantile_representatives)) {
        # Show message if no data
        plot.new()
        text(0.5, 0.5, values$quantile_representatives$message, cex = 1.5)
      } else {
        plot_quantile_tax_impacts(values$quantile_representatives)
      }
    })

    output$quantile_change_plot <- renderPlot({
      req(values$quantile_representatives)
      ...
      if ("message" %in% names(values$quantile_representatives)) {
        ...
        plot_quantile_tax_changes(values$quantile_representatives)
      }
    })

    # Detailed table
    output$quantile_representatives_table <- DT::renderDataTable({
      req(values$quantile_representatives)

      if ("message" %in% names(values$quantile_representatives)) {
        return(DT::datatable(values$quantile_representatives))
      }

      formatted_table <- format_quantile_representatives_table(
        values$quantile_representatives
      )
    ...
    # Property cards
    output$quantile_property_cards <- renderUI({
      req(values$quantile_representatives)
      ...
      if ("message" %in% names(values$quantile_representatives)) {
        return(div(
          ...
          " ",
          values$quantile_representatives$message
        ))
      }

      # Create a card for each representative property
      cards <- lapply(1:nrow(values$quantile_representatives), function(i) {
        prop <- values$quantile_representatives[i, ]
        ...
        box(
          ...
          fluidRow(
            ...
            column(
              6,
              h5(icon("dollar-sign"), "Existing Scenario"),
              p(
                strong("Property Value:"),
                scales::comma(round(prop$existing_value, 0))
              ),
              p(
                strong("Property Tax:"),
                scales::comma(round(prop$existing_property_tax, 0))
              ),
              p(
                strong("Business License:"),
                scales::comma(round(prop$existing_business_license, 0))
              ),
              p(
                strong("Total Tax:"),
                scales::comma(round(prop$existing_total_tax, 0))
              ),
              p(
                strong("Effective Rate:"),
                paste0(round(prop$existing_effective_rate, 2), "%")
              )
            )
          ),
          ...
          fluidRow(
            column(
              6,
              h5(icon("chart-line"), "Scenario A Impact"),
              p(
                strong("New Tax:"),
                scales::comma(round(prop$scenario_a_total_tax, 0))
              ),
              p(
                strong("Tax Change:"),
                span(
                  paste0(
                    ifelse(prop$scenario_a_tax_change >= 0, "+", ""),
                    scales::comma(round(prop$scenario_a_tax_change, 0))
                  ),
                  style = paste0(
                    "color: ",
                    ifelse(
                      prop$scenario_a_tax_change > 0,
                      "#e74c3c",
                      ifelse(
                        prop$scenario_a_tax_change < 0,
                        "#27ae60",
              ...
              p(
                ...
                span(
                  paste0(
                    ifelse(prop$scenario_a_tax_change_pct >= 0, "+", ""),
                    round(prop$scenario_a_tax_change_pct, 1),
                    "%"
                  ),
                  style = paste0(
                    "color: ",
                    ifelse(
                      prop$scenario_a_tax_change_pct > 0,
                      "#e74c3c",
                      ifelse(
                        prop$scenario_a_tax_change_pct < 0,
                        "#27ae60",
            ...
            column(
              6,
              h5(icon("chart-line"), "Scenario B Impact"),
              p(
                strong("New Tax:"),
                scales::comma(round(prop$scenario_b_total_tax, 0))
              ),
              p(
                strong("Tax Change:"),
                span(
                  paste0(
                    ifelse(prop$scenario_b_tax_change >= 0, "+", ""),
                    scales::comma(round(prop$scenario_b_tax_change, 0))
                  ),
                  style = paste0(
                    "color: ",
                    ifelse(
                      prop$scenario_b_tax_change > 0,
                      "#e74c3c",
                      ifelse(
                        prop$scenario_b_tax_change < 0,
                        "#27ae60",
              ...
              p(
                ...
                span(
                  paste0(
                    ifelse(prop$scenario_b_tax_change_pct >= 0, "+", ""),
                    round(prop$scenario_b_tax_change_pct, 1),
                    "%"
                  ),
                  style = paste0(
                    "color: ",
                    ifelse(
                      prop$scenario_b_tax_change_pct > 0,
                      "#e74c3c",
                      ifelse(
                        prop$scenario_b_tax_change_pct < 0,
                        "#27ae60",
    ...
    output$multi_type_properties <- DT::renderDataTable({
      ...
      multi_type <- analyze_multi_type_properties(revenue_data()[[
        input$analysis_scenario
      ]])
    ...
    output$concentration_metrics <- renderPrint({
      ...
      cat(
        "Top 1% of properties pay:",
        round(values$concentration_results$top_1pct, 1),
        "% of total tax\n"
      )
      cat(
        "Top 5% of properties pay:",
        round(values$concentration_results$top_5pct, 1),
        "% of total tax\n"
      )
      cat(
        "Top 10% of properties pay:",
        round(values$concentration_results$top_10pct, 1),
        "% of total tax\n"
      )
      cat(
        "Top 20% of properties pay:",
        round(values$concentration_results$top_20pct, 1),
        "% of total tax\n"
      )
    })
